...
const productsData = JSON.parse(fs.readFileSync('./data/articles.json', 'utf-8'));
...
app.put('/api/checkout', (req, res) => {
    try {
        ...
        for (const item of cart) {
            ...
            if (product.lagersaldo < item.quantity) {
                return res.status(400).json({ message: 'Otillräckligt lagersaldo för ' + product.name });
            }

            // update lagersaldot
            product.lagersaldo -= item.quantity;
        }
